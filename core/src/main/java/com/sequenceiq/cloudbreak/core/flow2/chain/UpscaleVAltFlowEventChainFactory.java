...
import com.sequenceiq.cloudbreak.core.flow2.cluster.upscalevalt.ClusterUpscaleVAltEvents;
...
public class UpscaleVAltFlowEventChainFactory implements FlowEventChainFactory<StackAndClusterUpscaleTriggerEvent> {
    ...
    public String initEvent() {
        return FlowChainTriggers.FULL_UPSCALE_TRIGGER_V_ALT_EVENT;
    }
    ...
    private void addClusterScaleTriggerEventIfNeeded(StackAndClusterUpscaleTriggerEvent event, StackView stackView, ClusterView clusterView,
        ...
        if (ScalingType.isClusterUpScale(event.getScalingType()) && clusterView != null) {
            ...
            flowEventChain.add(
                    new ClusterScaleVAltTriggerEvent(
                            ClusterUpscaleVAltEvents.CLUSTER_UPSCALE_VALT_TRIGGER_EVENT.event(),
                            stackView.getId(),
                            hostGroup.getName(),
                            event.getAdjustment(),
