...
# Hack - to simulate CM remembering keytabs: Make sure a directory to temporarily store keytabs exists
if [ -d "/tmp/kt/" ]; then
        echo "ZZZ: kt dir already exists"
else
        echo "ZZZ: Creating kt directory"
        mkdir "/tmp/kt"
fi
...
KEYTAB_PATH=/tmp/kt/${PRINC}_${HOST}.keytab
# Hack - Check if they keytab already exists. If it does, skip everything else
if [ -e $KEYTAB_PATH ]; then
  echo "ZZZ: Keytab $KEYTAB_PATH exists... using"
  CMD="cp $KEYTAB_PATH $KEYTAB_OUT"
  eval $CMD
  echo "ZZZ: Copied existing keytab to $KEYTAB_OUT ... Exiting with success"
  exit 0
else
  echo "ZZZ: Keytab $KEYTAB_PATH does not eixst. Falling back to regular flow"
fi
...
if [ ! -e $KEYTAB_PATH ] ; then
  ...
  CMD="cp $KEYTAB_PATH $KEYTAB_OUT"
  eval $CMD
  echo "ZZZ: Generated keytab $KEYTAB_PATH ... skipping deletion temporarily"
  #rm -f $KEYTAB_PATH
fi
