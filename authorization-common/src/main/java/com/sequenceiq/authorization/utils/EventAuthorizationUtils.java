...
public class EventAuthorizationUtils {
    ...
    private void checkPermissionForResource(AuthorizationResourceAction action, String resourceCrn) {
        List<AuthorizationResourceAction> specialActions = action.getSpecialActionRequirements();
        if (specialActions.isEmpty()) {
            checkPermission(action, resourceCrn);
        } else {
            boolean hasPermission = false;
            for (int i = 0; !hasPermission && i < specialActions.size(); i++) {
                try {
                    checkPermission(specialActions.get(i), resourceCrn);
                    hasPermission = true;
                } catch (AccessDeniedException ade) {
                    LOGGER.info(AccessDeniedException.class.getSimpleName() + " has caught during permission check for [action: {}, resourceCrn: {}]",
                            action.name(), resourceCrn);
                }
            }
            if (!hasPermission) {
                permissionCheckingUtils.throwAccessDeniedIfActionNotAllowed(action, Set.of(resourceCrn));
            }
        }
    }
